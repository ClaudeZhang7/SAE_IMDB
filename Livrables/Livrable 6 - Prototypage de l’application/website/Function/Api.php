<?php
    ...
    function get_data_by_id(String $id){
        $curl = curl_init();
    ...
    function get_data_by_name(String $name){
        $curl = curl_init();
        curl_setopt_array($curl, array(
    ...
    function get_poster_by_id(String $id){
        $data = get_data_by_id($id);
        $data_id = (array) $data;
        $array = (array) $data_id["result"];
        echo "<div class='block'><img class='image' src='" . $array["Poster"] . "' /><h3>" . $array["Title"] . "</h3></div>";
    }

    function get_poster_by_name(String $name){
    ...
    function get_five_poster(int $id_debut){
    ...
    function get_many_poster(int $id_debut, int $number_poster){
        for ($i=$id_debut; $i < $id_debut+$number_poster; $i++) { 
            $id = "tt000000" . strval($i);
            get_poster_by_id($id);
        }
    }
